...
import gd.app.musicplayer.ui.feature.menu.SortByContextMenu
...
class ActivityMusicSelect : BaseActivity(), View.OnClickListener, TextWatcher,
    ...
    private val preferenceUtil by lazy { appContainer.preferenceUtil }
...
//    }
    ...
    private fun handleLoadCompleted(event: MusicSelectAsyncEvent.LoadCompleted) {
        ...
        lockedSongIds = event.result.targetSetSongs.mapTo(hashSetOf(), Music::_id)
        selectedSongIds.retainAll(allLoadedSongs.mapTo(hashSetOf(), Music::_id))
    ...
    private fun refreshSelectionUi() {
        ...
        binding.mainInfoSelectall.renderSelectAllState(
            SelectionUiState(
                selectedCount = count,
                hasSelectableItems = visibleSongs.any { it._id !in lockedSongIds },
    ...
    private fun toggleSongSelection(music: Music) {
        if (music._id in lockedSongIds) return
        if (!selectedSongIds.add(music._id)) {
            selectedSongIds.remove(music._id)
        }
    ...
    private fun confirmSelection() {
        ...
        val selectedSongs = allLoadedSongs.filter { it._id in selectedSongIds }
