...
class ActivityDuplicateViewModel @Inject constructor(
    ...
    fun deleteSelected() {
        ...
        viewModelScope.launch {
            ...
            val selectedTracks = state.groups
                ...
                .filter { it._id in state.selectedIds }
                .distinctBy(Music::_id)
                .toList()
            ...
            if (deletedCount > 0) {
                val deletedIds = selectedTracks.mapTo(hashSetOf(), Music::_id)
                _uiState.update { current ->
                    val updatedGroups = current.groups
                        .mapNotNull { group ->
                            val remainingTracks = group.tracks.filterNot { it._id in deletedIds }
    ...
    private fun startScan() {
        ...
        scanJob = viewModelScope.launch {
            val tracks = withContext(dispatchers.io) {
                libraryRepo.observeTracks(MusicSet.Tracks).first().distinctBy(Music::_id)
            }
    ...
    private suspend fun findDuplicateGroups(
        ...
        return duplicateHashes.mapIndexed { index, hash ->
            ...
            DuplicateGroup(
                key = "${hash}_${index}_${groupTracks.first()._id}",
                tracks = groupTracks,
                expanded = false
            )
        }
    ...
    private fun buildDefaultSelection(groups: List<DuplicateGroup>): Set<Long> {
        return buildSet {
            groups.forEach { group ->
                group.tracks.drop(1).forEach { add(it._id) }
            }
        }
    }
