...
import gd.app.musicplayer.core.ui.extension.extractValidatedText
...
import com.ijoysoft.music.model.soundclip.SoundWaveData
...
class ActivityAudioEditor : BaseActivity(),
    ...
    override fun onInputTimeChanged(timeEditText: TimeEditText, rawText: String, timeMs: Int) {
        if (timeEditText === binding.audioEditorStartTime) {
            ...
            if (adjusted != timeMs) {
                binding.audioEditorStartTime.setText(TimeEditText.c(adjusted))
            }
            binding.audioEditorWave.k(adjusted, false)
            ...
            if (fallback != timeMs) {
                binding.audioEditorEndTime.setText(TimeEditText.c(fallback))
            }
    ...
    override fun F(timeMs: Int) {
        val safeTime = timeMs.coerceAtLeast(0)
        binding.audioEditorEndTime.setText(TimeEditText.c(safeTime))
        updateLengthText()
    }

    override fun H(timeMs: Int) {
        val safeTime = timeMs.coerceAtLeast(0)
        binding.audioEditorStartTime.setText(TimeEditText.c(safeTime))
        updateLengthText()
    }

    override fun s(timeMs: Int) {
    ...
    private fun bindViews() {
        ...
        binding.audioEditorZoomIn.setOnClickListener {
            binding.audioEditorWave.r()
            refreshZoomButtons()
        }
        binding.audioEditorZoomOut.setOnClickListener {
            binding.audioEditorWave.s()
            refreshZoomButtons()
        }
        ...
        binding.audioEditorStartMinus.setOnClickListener {
            binding.audioEditorWave.k(max(0, binding.audioEditorWave.getClipLeftMilliseconds() - 10), false)
            H(binding.audioEditorWave.getClipLeftMilliseconds())
        }
        binding.audioEditorStartPlus.setOnClickListener {
            binding.audioEditorWave.k(
                min(
        ...
        binding.audioEditorEndMinus.setOnClickListener {
            ...
            F(binding.audioEditorWave.getClipRightMilliseconds())
        }
    ...
    private fun loadWaveform() {
        ...
        lifecycleScope.launch {
            ...
            binding.audioEditorStartTime.setText(TimeEditText.c(0))
            binding.audioEditorEndTime.setText(TimeEditText.c(loaded.durationMs))
            updateLengthText()
            refreshZoomButtons()
            ensurePlayer()
    ...
    private fun markStartFromPlayback() {
        ...
        binding.audioEditorWave.k(position, false)
        H(position)
    }
    ...
    private fun refreshZoomButtons() {
        binding.audioEditorZoomIn.isEnabled = binding.audioEditorWave.c()
        binding.audioEditorZoomOut.isEnabled = binding.audioEditorWave.d()
    }
    ...
    private fun requestSave() {
        ...
        if (binding.audioEditorStartTime.isFocused) binding.audioEditorStartTime.e()
        if (binding.audioEditorEndTime.isFocused) binding.audioEditorEndTime.e()
            ...
            if (insertedId != null) {
                withContext(Dispatchers.IO) {
                    MediaStoreMusicImporter().queryMusicById(this@ActivityAudioEditor, insertedId)
                        ?.let { appContainer.scanRepo.upsertTracks(listOf(it)) }
                }
    ...
    private fun setEditorEnabled(enabled: Boolean) {
        ...
        binding.audioEditorZoomIn.isEnabled = enabled && binding.audioEditorWave.c()
        binding.audioEditorZoomOut.isEnabled = enabled && binding.audioEditorWave.d()
    ...
    private fun updateLengthText() {
        binding.audioEditorLength.text = TimeEditText.c(binding.audioEditorWave.getClipDuration())
    }
