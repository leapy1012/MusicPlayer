...
class EqualizerFragment : ViewBindingFragment<FragmentEqualizerBinding>(), View.OnClickListener {
    ...
    private fun showPresetPicker() {
        ...
        ChoiceListDialog.show(requireActivity(), config)
    }
...
//                        context.sp(16f)
    ...
    private fun themedMessageDialogConfig(): MessageDialog.Config {
        val palette = requireActivity().appContainer.themeRepo
    ...
    private fun themedSelectableListDialogConfig(items: List<String>): ChoiceListDialog.Config {
        val palette = requireActivity().appContainer.themeRepo
            .getCorePalette(requireContext())
        return ChoiceListDialog.Config.create(requireContext(), items).apply {
            titleTextColor = palette.titleColor
    ...
    private fun themedPlainListDialogConfig(items: List<String>): ChoiceListDialog.Config {
        val palette = requireActivity().appContainer.themeRepo
            .getCorePalette(requireContext())
        return ChoiceListDialog.Config.create(requireContext(), items).apply {
            titleTextColor = palette.titleColor
