...
class LockActivity : BaseActivity(),
    ...
    private val toggleFavoriteTrack by lazy { appContainer.toggleFavoriteTrackUseCase }
    ...
    private fun bindListeners() {
        ...
        findViewById<View>(R.id.lock_more).setOnClickListener(this)
        findViewById<View>(R.id.lock_play_favourite).setOnClickListener(this)
        findViewById<View>(R.id.lock_play_queue).setOnClickListener(this)
        findViewById<View>(R.id.control_mode).setOnClickListener(this)
        findViewById<View>(R.id.control_previous).setOnClickListener(this)
        findViewById<View>(R.id.control_play_pause).setOnClickListener(this)
        findViewById<View>(R.id.control_next).setOnClickListener(this)
    }
    ...
    private fun updateBackground(track: Music) {
        if (preferenceUtil.getIntPreference(KEY_LOCK_BACKGROUND, 1) == 1) {
            Glide.with(this)
                .load(
                    track.albumPicture?.takeIf { it.isNotBlank() }
                        ?: track.album_id.takeIf { it.isNotBlank() }
            ...
            backgroundImage.setImageDrawable(
                appContainer.themeRepo.getCorePalette(this).getActivityBackgroundDrawable(this)
            )
        }
    }
    ...
    private fun toggleFavorite() {
        ...
        lifecycleScope.launch {
            favoriteView.isSelected = toggleFavoriteTrack(track._id)
        }
    }

    private fun showTrackOptions() {
        val track = currentTrack ?: return
        MusicOptionsDialog.Companion.newInstance(track, MusicSet.Tracks)
