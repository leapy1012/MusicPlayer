...
import gd.app.musicplayer.ui.feature.lyrics.FullLyricDialogFragment
...
class FragmentPlayContent : ViewBindingFragment<FragmentPlayContentBinding>(), View.OnClickListener,
    ...
    private fun setupPager() {
        ...
        lyricView.setOnClickListener {
            if (lyricView.a()) {
                FullLyricDialogFragment.show(childFragmentManager)
            }
        }
    ...
    private fun maybeLoadLyrics(track: Music) {
        ...
        lyricLoadJob?.cancel()
        lyricView.setTimeOffset(TrackLyricsStore.from(requireContext()).getTrackLyricOffset(track.id))
    ...
    override fun onClick(view: View) {
        when (view.id) {
            ...
            R.id.control_mode -> {
                viewModel.cyclePlayMode(requireContext())
                refreshPlayModeIcon()
            }
    ...
    private fun openLyricSearch() {
        ...
        LyricSearchDialogFragment.newInstance(track.id, track.title, track.artist, track.data)
    ...
    private fun openLyricSettings() {
        ...
        LyricSettingsDialogFragment.newInstance(track.id, lyricView.hasTimedLyrics())
    ...
    private fun refreshPlayModeIcon() {
        controlMode.setImageResource(
            when (PreferenceUtil.getInstance(requireContext()).getPlayMode()) {
