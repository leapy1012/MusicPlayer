...
object MaterialDialogConfig {
    ...
    fun createMaterialListDialogConfig(
        context: Context,
        items: List<String>
    ): ChoiceListDialog.Config {
        val palette = context.appContainer.themeRepo.getCorePalette(context)
        val colors = DialogThemeColors.from(
            isDarkMode = palette.isDarkMode(),
            accentColor = palette.accentColor
        )

        return ChoiceListDialog.Config.create(context, items).apply {
    ...
    private fun ChoiceListDialog.Config.applyMaterialActionButtons(
