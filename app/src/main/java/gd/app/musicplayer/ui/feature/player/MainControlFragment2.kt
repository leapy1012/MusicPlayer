...
@AndroidEntryPoint
class MainControlFragment2 : ViewBindingFragment<FragmentMainControl2Binding>(),
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.playbackState.collect { state ->
                    ...
                    binding.mainControlProgress.isEnabled = track.data.isNullOrBlank().not()
                    if (!userSeeking) {
    ...
    private fun toggleFavorite() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            favoriteOverride = toggleFavoriteTrack(track.id)
